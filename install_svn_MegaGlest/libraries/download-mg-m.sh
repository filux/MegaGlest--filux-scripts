...
if [ ! -d "$path" ]; then mkdir "$path"; fun_err er2; sleep 0.5s; fi; fun_mk () { if [ ! -d "$path""$spx" ]; then mkdir "$path""$spx"; fun_err er2; sleep 0.5s; fi } 
fun_get () { 
  if [ "$spx" = "maps/" ]; then sx=""; else sx="$dsx"; fi; IFS='|'
  while read; do read -ra array <<< "$REPLY"; if [ "$spx" = "tilesets/" ] || [ "$spx" = "scenarios/" ]; then addr="${array[3]}"; else addr="${array[4]}"; fi; name="${array[0]}"
    wget -O "$name""$sx" -c "$addr"; fun_err er3; echo -e " -- V --\n"; sleep 0.2s; if [ "$spx" = "maps/" ] && [ ! -f "$name" ]; then sleep 1s; fi
    if [ ! -d "$name" ] && [ -f "$name$dsx" ]; then 7z x -y "$name""$dsx"; fun_err er1; echo -e "\n -- V --\n"; sleep 0.2s; fi
  done< <(echo "$files"); sleep 1s; unset IFS
}
spx="maps/"; fun_mk; cd "$path""$spx"; files="$ma_fs"; fun_get; spx="tilesets/"; fun_mk; cd "$path""$spx"; files="$ti_fs"; fun_get
spx="techs/"; fun_mk; cd "$path""$spx"; files="$te_fs"; fun_get; spx="scenarios/"; fun_mk; cd "$path""$spx"; files="$sc_fs"; fun_get; exit 0