...
cnetg="defaultNetworkGameSetup.mgg"; opcnetg="$pwd0""$cnetg"; pcnetg="$pwd2""data/glest_game/data/"; builddeps="setupBuildDeps.sh"
builddepsCL="./setupBuildDeps.sh --finstall"; count=0; ssize=1025
...
fun_err() { if [ "$?" -ne "0" ]; then echo -e "\n >>> an error was detected <<< "
  ...
  cd "$pwd0"; fun_err er4; chmod +x "$pwd0""$builddeps"; fun_err er2; echo -e " Please enter the root's password.\n"; sleep 0.3s
echo -n " "; echo -e "$pwd0>$builddepsCL\n"; echo -n " "; sleep 0.3s; sudo "$pwd0"$builddepsCL; fun_err er1
