...
if [ -x "$ZENITY" ] || [ -x "$KDIALOG" ]; then (
	...
	(while [ "$count" -lt "101" ]; do 
		...
		if [ "$asize" -lt "1" ]; then asize=1; fi

		count=$(($asize * 100 / $ssize))

		if [ -x "$ZENITY" ]; then echo $count
...
sleep 0.3s
$svnc; if [ "$?" -ne "0" ]; then sleep 2s; svn cleanup; sleep 5s; $svnc; fun_err er3; fi

if [ -f "$scrfile" ]; then 
	cp "$scrfile" "$pwd2"; fun_err er2
	cd "$pwd2"; fun_err er4
	sleep 0.3s
	chmod +x "$script"; fun_err er2
fi

sleep 0.3s
echo -e "\n\n$pwd2>./$script -fh"; ./"$script" -fh; fun_err er1
cp "$opcnetg" "$pcnetg"; fun_err er2
