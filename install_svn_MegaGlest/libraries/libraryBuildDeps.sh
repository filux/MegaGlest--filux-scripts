#!/bin/bash
#fmod#
#fmod# This script is used as a "library".
#
#fmod# This script installs build dependencies on a number of Linux platforms.
# ----------------------------------------------------------------------------
# Originally written by Mark Vejvoda <mark_vejvoda@hotmail.com>
# Rewritten by Tom Reynolds <tomreyn@megaglest.org>
# Small modifications and rename from "setupBuildDeps.sh" to "libraryBuildDeps.sh" (04.2013) by filux <heross(@@)o2.pl>
# Copyright (c) 2013 Mark Vejvoda, Tom Reynolds, filux under GNU GPL v3.0
LANG=C
...
#fmod## Do you have the 'svnversion' command?
#fmod#if [ `which svnversion`'x' = 'x' ] 
#fmod#then
#fmod#	echo 'Could not find "svnversion", please make sure it is installed.' >&2
#fmod#	exit 1
#fmod#fi

#fmod#svnversion=`readlink -f $0 | xargs dirname | xargs svnversion`
...
then
	lsb=0
	#fmod#start
	if [ -e /etc/debian_version ];     then distribution='Debian';   release='unknown release version'; codename=`cat /etc/debian_version`
	elif [ -e /etc/SuSE-release ];     then distribution='SuSE';     release='unknown release version'; codename=`cat /etc/SuSE-release`
	elif [ -e /etc/fedora-release ];   then distribution='Fedora';   release='unknown release version'; codename=`cat /etc/fedora-release`
	elif [ -e /etc/redhat-release ];   then distribution='Redhat';   release='unknown release version'; codename=`cat /etc/redhat-release`
	elif [ -e /etc/mandrake-release ]; then distribution='Mandrake'; release='unknown release version'; codename=`cat /etc/mandrake-release`
...
unsupported_distribution () {
	...
	echo '--- snip ---'
	#fmod#echo 'SVN version:  '"$svnversion"
	...
	SuSE|SUSE?LINUX|Opensuse) 
		case $release in
			11.2|11.3|11.4|12.1)
				...
				;;
			12.2|12.3)
				#fmod#start tested on latest 12.2, at least on the early 12.3 should also be ok 
				installcommand='zypper install subversion gcc gcc-c++ automake cmake libSDL-devel libxerces-c-devel Mesa-libGL-devel freeglut-devel libvorbis-devel wxWidgets-devel lua-devel libjpeg62-devel libpng14-compat-devel libcurl-devel openal-soft-devel libX11-devel libxml2-devel libircclient-devel glew-devel ftgl-devel fribidi-devel libminiupnpc-devel vlc-devel libjpeg-turbo help2man'
				#fmod#stop
				$installcommand
				if [ $? != 0 ]; then error_during_installation; exit 1; fi
				;;
