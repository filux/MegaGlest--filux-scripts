...
fun_1() {
	pidof megaglest &>/dev/null
	if [ "$?" -eq "0" ] && [ "$freshex" -ne "1" ]; then 
		sleep "$x1"s
		echo -e "\n Game client is already running. >>> Switches tasks to the "'"fresh"'" mode."
		freshex=1
		sleep "$x1"s
	fi
	...
	if [ "$freshex" -ne "1" ]; then
		...
		if [ "$verboseex" -eq "1" ]; then
			...
			sleep "$x1"s
			echo -e " >>> script has started the headless server <<<\n `date`\n" > "$logfileh"; fun_0 er2
			sleep 0.05s
			./megaglest --version |& tee -a "$logfileh"; fun_0 er3
			sleep 0.05s
			echo " " >> "$logfileh"
			sleep 1s
		...
		if [ -f megaglest ]; then
			echo " "; ./megaglest --version; fun_0 er3
		fi
		...
		if [ "$noconex" -eq "1" ]; then
			sleep "$x1"s
			echo -e " press enter key to continue/exit...\n\n"
			read -t25
		fi
	fi
} 
...
fun_2() { 
	...
	if [ -d "$builddir" ] && [ "$rebuildex" -ne "1" ]; then
		nrebrecord=`expr $nrebrecord + 1`
	else 
		nrebrecord=0
	fi
	
	rebrecord="$nrebrecord regular builds ago"
...
if [ "$helpex" -eq "1" ]; then
	...
	if [ ! -f "$FILE" ]; then
		...
		Terminal=true\nExec=/bin/bash ./megaglest-svn.sh -nc\nCategories=Game;StrategyGame;" > "$FILE"; fun_0 er2
		sleep 0.05s
		chmod +x "$FILE"; fun_0 er2
	fi
	
	if [ "$libcheckex" -eq "1" ]; then
		cd "$pwd3"; fun_0 er1
		...
		sleep "$x1"s
		echo -e " Verification has to be done with "'"administrator"'" rights. Please enter the root's password.\n"
		echo -n " "; sleep "$x2"s; su -c ./setupBuildDeps.sh; fun_0 er3
		echo -e "\n\n"
		sleep "$x3"s
		
		if [ -f "megaglest" ]; then
			echo -e " "$pwd3"> ./start_megaglest --version\n"
			sleep "$x2"s
			./start_megaglest --version; fun_0 er3
			echo -e "\n\n"
			sleep "$x3"s
		fi
	...
	cd "$pwd1"; fun_0 er1
	if [ "$rebuildex" -eq "1" ] && [ -d "$builddir" ]; then rm -r "$builddir"; fun_0 er1; fi
	
	if [ -f "$fullrebfile" ]; then
		read rebrecord < "$fullrebfile"
		sleep 0.05s
		nrebrecord=`expr "$rebrecord" : '\([0-9]*[0-9]\)'`
	else 
	...
	if [ "$conect" -ne "1" ]; then
		...
		if [ "$rezultl" -lt "999" ]; then
			serverv=`expr "$rezult" : '.*\(.....\)'`
			rezult2=$(echo "$rezult" | sed -e '$d' -e "s|$serverv||" -e 's/ //g')
			rezult3=$(echo "$rezult2" | sed -e "s|$exc||" -e "s|$exc2||" -e "s|$exc3||" -e '/^$/d')
			rezultl2=${#rezult3}
			
			if [ "$rezultl2" -eq "0" ]; then 
				echo -e " >>> Yes, it is. <<<"
				if [ ! -d "$builddir" ]; then
					if [ "$rebuildex" -ne "1" ]; then
						sleep "$x1"s
